    ...
    try {
      ...
      setChatMessages((prev) => [
        ...prev,
        {
          ...
          routingReport: res.routingReport,
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2">
            {chatMessages.map((msg, i) => (
              ...
              >
                ...
                <div
                  className={`p-4 rounded-xl max-w-[88%] leading-relaxed space-y-3.5 ${
                    msg.sender === 'user'
                      ? 'bg-indigo-600 text-white font-medium shadow-xs'
                      : 'bg-slate-50/90 border border-slate-200 text-slate-800 shadow-2xs'
                  }`}
                >
                  {/* Resposta Textual com Parágrafos Claramente Separados */}
                  <div className="space-y-2.5 text-xs text-slate-800 leading-relaxed">
                    {msg.text
                      ...
                      .replace(/\\\)/g, ')')
                      .replace(/\\([*_{}[\]()#+\-.!])/g, '$1')
                      .replace(/\bDr\.\s*Gabriela\b/g, 'Dra. Gabriela')
                      .split(/\n\s*\n/)
                      .map((para, pIdx) => (
                  ...
                  {/* 1. Card Separado: Rota de Competência Material e Jurisdicional */}
                  {msg.routingReport && (
                    <div className="pt-2 border-t border-slate-200">
                      <div className="p-3 rounded-lg bg-white border border-slate-200 text-xs space-y-2.5 shadow-2xs">
                        <div className="flex items-center gap-1.5 font-bold text-slate-800 pb-1.5 border-b border-slate-100">
                          <Scale className="w-3.5 h-3.5 text-indigo-600 flex-shrink-0" />
                          <span>Roteamento por Competência Jurisdicional (CF/88)</span>
                        </div>

                        {/* Fontes Excluídas por Incompetência Material */}
                        {msg.routingReport.sourcesExcluded && msg.routingReport.sourcesExcluded.length > 0 && (
                          <div className="space-y-1.5">
                            <span className="text-[10px] font-bold uppercase tracking-wider text-rose-700 block">
                              Fontes Excluídas da Pesquisa por Incompetência:
                            </span>
                            {msg.routingReport.sourcesExcluded.map((ex: any, exIdx: number) => (
                              <div
                                key={exIdx}
                                className="p-2 rounded-md bg-rose-50 border border-rose-200 text-rose-900 text-[11px] flex items-start gap-2"
                              >
                                <span className="font-mono text-[9px] px-1.5 py-0.5 rounded bg-rose-200 text-rose-950 font-bold flex-shrink-0">
                                  {ex.courtCode || 'EXCLUÍDO'} • {ex.lifecycleState || 'EXCLUDED_BY_JURISDICTION'}
                                </span>
                                <span className="leading-snug text-[10.5px]">{ex.reason}</span>
                              </div>
                            ))}
                          </div>
                        )}

                        {/* Fontes Competentes Ainda Não Implementadas */}
                        {msg.routingReport.sourcesNotImplemented && msg.routingReport.sourcesNotImplemented.length > 0 && (
                          <div className="space-y-1.5">
                            <span className="text-[10px] font-bold uppercase tracking-wider text-amber-800 block">
                              Fontes Competentes Pendentes de Conector:
                            </span>
                            {msg.routingReport.sourcesNotImplemented.map((ni: any, niIdx: number) => (
                              <div
                                key={niIdx}
                                className="p-2 rounded-md bg-amber-50/80 border border-amber-200 text-amber-950 text-[11px] flex items-start gap-2"
                              >
                                <span className="font-mono text-[9px] px-1.5 py-0.5 rounded bg-amber-200 text-amber-950 font-bold flex-shrink-0">
                                  {ni.courtCode} • {ni.lifecycleState || 'SOURCE_NOT_IMPLEMENTED'}
                                </span>
                                <span className="leading-snug text-[10.5px]">
                                  <strong>{ni.name}:</strong> {ni.message}
                                </span>
                              </div>
                            ))}
                          </div>
                  ...
                  {/* 2. Card Separado: Telemetria Técnica e Diagnóstico da Fonte Oficial */}
                  {msg.diagnostic && (
                    <div className="pt-2 border-t border-slate-200">
                      <div className="p-3 rounded-lg bg-white border border-slate-200 text-xs space-y-2.5 font-mono shadow-2xs">
                        <div className="flex items-center justify-between pb-1.5 border-b border-slate-100 flex-wrap gap-1">
                          <span className="flex items-center gap-1.5 font-bold text-slate-800 text-[11px]">
                            ...
                            {msg.diagnostic.sourceName || (msg.diagnostic.adapter === 'stj-dados-abertos' ? 'STJ - Portal de Dados Abertos (SCON/CKAN)' : msg.diagnostic.adapter)}
                          </span>
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className={`px-2 py-0.5 rounded text-[9.5px] font-bold ${
                              msg.diagnostic.lifecycleState === 'SEARCH_SUCCESS' || msg.diagnostic.lifecycleState === 'PARSE_SUCCESS' || msg.diagnostic.lifecycleState === 'INDEX_SUCCESS'
                                ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                                : msg.diagnostic.lifecycleState === 'SOURCE_NOT_IMPLEMENTED'
                                ? 'bg-blue-50 text-blue-700 border border-blue-200'
                                : msg.diagnostic.lifecycleState === 'PARSER_EMPTY' || msg.diagnostic.lifecycleState === 'EMPTY_VALID_DATASET'
                                ? 'bg-amber-50 text-amber-700 border border-amber-200'
                                : 'bg-rose-50 text-rose-700 border border-rose-200'
                            }`}>
                              {msg.diagnostic.lifecycleState || 'TELEMETRIA'}
                            </span>
                            <span className={`px-1.5 py-0.5 rounded text-[9.5px] font-bold ${msg.diagnostic.httpStatus === 200 ? 'bg-slate-100 text-slate-700 border border-slate-200' : 'bg-rose-100 text-rose-800 border border-rose-200'}`}>
                              HTTP {msg.diagnostic.httpStatus} • {msg.diagnostic.latencyMs}ms
                            </span>
                          </div>
                        </div>

                        {msg.diagnostic.stateDescription && (
                          <div className="p-1.5 rounded bg-slate-50 text-[10px] text-slate-700 font-sans leading-relaxed border border-slate-100">
                            {msg.diagnostic.stateDescription}
                          </div>
                        )}

                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10.5px] text-slate-600 pt-0.5">
                          <div>
                            <span className="text-slate-400">Registros lidos:</span>{' '}
                            <span className="font-bold text-slate-800">{(msg.diagnostic.recordsRead ?? msg.diagnostic.documentsReceived ?? 0).toLocaleString('pt-BR')}</span>
                          </div>
                          <div>
                            <span className="text-slate-400">Normalizados:</span>{' '}
                            <span className="font-bold text-slate-800">{msg.diagnostic.documentsNormalized ?? 0}</span>
                          </div>
                          {msg.diagnostic.bytesTransferred !== undefined && msg.diagnostic.bytesTransferred > 0 && (
                            <div>
                              <span className="text-slate-400">Bytes transferidos:</span>{' '}
                              <span className="font-bold text-slate-800">{(msg.diagnostic.bytesTransferred / 1024).toFixed(1)} KiB</span>
                            </div>
                          )}
                          {msg.diagnostic.normalizedQueryNumber && (
                            <div>
                              <span className="text-slate-400">Termo Normalizado:</span>{' '}
                              <span className="font-bold text-slate-800">{msg.diagnostic.normalizedQueryNumber}</span>
                            </div>
                          )}
                        </div>

                        {msg.diagnostic.officialUrl && (
                          <div className="text-[10px] text-slate-500 truncate pt-1.5 border-t border-slate-100">
                            <span className="text-slate-400">Recurso Oficial:</span>{' '}
                            <a href={msg.diagnostic.officialUrl} target="_blank" rel="noopener noreferrer" className="text-indigo-600 hover:underline">
                              {msg.diagnostic.officialUrl}
                            </a>
                          </div>
                        )}
                  ...
                  {/* 3. Card Separado: Controle de Pertinência Ativo (Fail-Closed) */}
                  {msg.status === 'FAIL_CLOSED' && (
                    <div className="pt-2 border-t border-slate-200">
                      <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-900 space-y-1.5 text-[11px] shadow-2xs">
                        <div className="flex items-center gap-1.5 font-bold text-amber-900">
                          ...
                          <span className="ml-auto font-mono text-[9px] px-1.5 py-0.5 rounded bg-amber-200/80 text-amber-950 font-bold">
                  ...
                  {/* 4. Card Separado: Precedentes Oficiais Vinculados */}
                    ...
                    <div className="pt-2 border-t border-slate-200 space-y-2.5">
                      ...
                      <div className="space-y-2">
                        {msg.searchResults.map((sr: any) => (
                          <div
                            key={sr.id}
                            className="p-3 rounded-lg bg-white border border-slate-200 shadow-2xs space-y-1.5 text-[11px]"
                          >
                            <div className="flex items-center justify-between gap-2 flex-wrap">
                              <span className="font-bold text-indigo-950 font-mono text-[11.5px]">{sr.caseNumber}</span>
                              <span className="px-2 py-0.5 rounded text-[9.5px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center gap-1">
                            ...
                            <div className="flex items-center justify-between pt-1.5 text-[10px] text-slate-500 border-t border-slate-100">
                              ...
                              {sr.officialUrl && (
                                <a
                                  ...
                                  className="text-indigo-600 hover:text-indigo-800 font-medium flex items-center gap-0.5 underline"
                                >
                                  Fonte Oficial
