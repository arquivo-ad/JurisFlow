  ...
  try {
    ...
    if (!cErr && cases && cases.length > 0) {
      const fetchedCases = cases.map((c: any) => ({
        ...
        responsibleLawyerName: db.users.find((u: any) => u.id === c.responsible_user_id)?.name || 'Dr. Carlos Silveira',
        tags: c.tags || [],
        deadlinesCount: 0,
    ...
    if (!dErr && deadlines && deadlines.length > 0) {
      const fetchedDeadlines = deadlines.map((d: any) => ({
        ...
        responsibleUserName: db.users.find((u: any) => u.id === d.assigned_user_id)?.name || 'Dr. Carlos Silveira',
        title: d.title,
        description: d.description,
