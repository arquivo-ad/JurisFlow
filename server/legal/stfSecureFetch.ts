...
const STF_HOST = 'portal.stf.jus.br';

// Official AIA intermediate presented by the STF leaf certificate.
...
export const secureStfFetch: typeof fetch = async (input: RequestInfo | URL, init: RequestInit = {}) => {
  const url = new URL(typeof input === 'string' || input instanceof URL ? input.toString() : input.url);
  if (url.hostname !== STF_HOST) return fetch(input, init);
  ...
  return await new Promise<Response>((resolve, reject) => {
    const request = https.request(url, {
      ...
      servername: STF_HOST,
      ca: [...tls.rootCertificates, GLOBALSIGN_GCC_R6_ALPHASSL_CA_2025],
    }, (res) => {
      const chunks: Buffer[] = [];
