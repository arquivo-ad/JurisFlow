...
import { StateCourtPortalDiagnosticAdapter, type DiagnosticCourtCode } from './adapters/StateCourtPortalDiagnosticAdapter.ts';
...
export class JudicialSearchService {
  ...
  private tseAdapter: TseJurisprudenciaAdapter;
  ...
  private tjesAdapter: TjesJurisprudenciaAdapter;
  private tjseAdapter: TjseJurisprudenciaAdapter;
  private statePortalDiagnosticAdapters: Record<DiagnosticCourtCode, StateCourtPortalDiagnosticAdapter>;
  ...
  constructor() {
    ...
    this.tseAdapter = new TseJurisprudenciaAdapter();
    ...
    this.tjesAdapter = new TjesJurisprudenciaAdapter();
    this.tjseAdapter = new TjseJurisprudenciaAdapter();
    this.statePortalDiagnosticAdapters = {
      TJPB: new StateCourtPortalDiagnosticAdapter('TJPB'),
      TJAP: new StateCourtPortalDiagnosticAdapter('TJAP'),
      TJMT: new StateCourtPortalDiagnosticAdapter('TJMT'),
    };
  }
  ...
  public async searchJurisprudence(params: JurisprudenceSearchParams, tenantId?: string) {
    ...
    const requestsTse = params.courtCodes?.includes('TSE') === true;
    ...
    const requestsTjes = params.courtCodes?.includes('TJES') === true;
    const requestsTjse = params.courtCodes?.includes('TJSE') === true;
    const requestedPortalDiagnosticCodes = (params.courtCodes || []).filter(
      (code): code is DiagnosticCourtCode => ['TJPB', 'TJAP', 'TJMT'].includes(code)
    );
    ...
    let tseDiagnostic = undefined as Awaited<ReturnType<TseJurisprudenciaAdapter['searchOfficialJurisprudence']>>['diagnostic'] | undefined;
    ...
    let tjesDiagnostic = undefined as Awaited<ReturnType<TjesJurisprudenciaAdapter['searchOfficialJurisprudence']>>['diagnostic'] | undefined;
    let tjseDiagnostic = undefined as Awaited<ReturnType<TjseJurisprudenciaAdapter['searchOfficialJurisprudence']>>['diagnostic'] | undefined;
    let statePortalDiagnostic = undefined as Awaited<ReturnType<StateCourtPortalDiagnosticAdapter['searchOfficialJurisprudence']>>['diagnostic'] | undefined;
    ...
    if (requestsTjce) {
      const tjceResult = await this.tjceAdapter.searchOfficialJurisprudence(params.query || params.caseNumber || '', 10);
      tjceDiagnostic = tjceResult.diagnostic;
      regionalSourcesConsulted.push('tjce-jurisprudencia');
      for (const decision of tjceResult.decisions) {
        if (decision.verificationStatus === 'VERIFIED_OFFICIAL') legalStorage.upsertDecision(decision);
      }
    }
    ...
    if (requestsTjto) {
      const tjtoResult = await this.tjtoAdapter.searchOfficialJurisprudence(params.query || params.caseNumber || '', 10);
      tjtoDiagnostic = tjtoResult.diagnostic;
      regionalSourcesConsulted.push('tjto-jurisprudencia');
    ...
    if (requestsTjes) {
      const tjesResult = await this.tjesAdapter.searchOfficialJurisprudence(params.query || params.caseNumber || '', 10);
      tjesDiagnostic = tjesResult.diagnostic;
      regionalSourcesConsulted.push('tjes-jurisprudencia');
      for (const decision of tjesResult.decisions) {
        if (decision.verificationStatus === 'VERIFIED_OFFICIAL') legalStorage.upsertDecision(decision);
      }
    }

    if (requestsTjse) {
      const tjseResult = await this.tjseAdapter.searchOfficialJurisprudence(params.query || params.caseNumber || '');
      tjseDiagnostic = tjseResult.diagnostic;
      regionalSourcesConsulted.push('tjse-jurisprudencia');
    }

    for (const courtCode of requestedPortalDiagnosticCodes) {
      const portalResult = await this.statePortalDiagnosticAdapters[courtCode]
        .searchOfficialJurisprudence(params.query || params.caseNumber || '');
      statePortalDiagnostic = statePortalDiagnostic ?? portalResult.diagnostic;
      regionalSourcesConsulted.push(`${courtCode.toLowerCase()}-jurisprudencia`);
    }
    ...
    if (requestsTse) {
      const tseResult = await this.tseAdapter.searchOfficialJurisprudence(params.query || params.caseNumber || '');
      tseDiagnostic = tseResult.diagnostic;
      nationalSourcesConsulted.push('tse-jurisprudencia');
    }
    ...
    if (requestsTjrr) {
      ...
      regionalSourcesConsulted.push('tjrr-jurisprudencia');
      for (const decision of tjrrResult.decisions) {
        if (decision.verificationStatus === 'VERIFIED_OFFICIAL') legalStorage.upsertDecision(decision);
      }
    }
    ...
    if (requestsTjpi) {
      const tjpiResult = await this.tjpiAdapter.searchOfficialJurisprudence(params.query || params.caseNumber || '', 10);
      tjpiDiagnostic = tjpiResult.diagnostic;
      regionalSourcesConsulted.push('tjpi-jurisprudencia');
      for (const decision of tjpiResult.decisions) {
        if (decision.verificationStatus === 'VERIFIED_OFFICIAL') legalStorage.upsertDecision(decision);
      }
    }
  ...
  public getAvailabilityMatrix(): CourtAvailabilityMatrixItem[] {
    ...
    return [
      ...
      {
        courtCode: 'TJMT', courtName: 'TJMT - Portal da Jurisprudência', jurisdiction: 'MT',
        jurisprudenceStatus: 'DISPONIVEL_PARCIAL', processStatus: 'DISPONIVEL_PUBLICO',
        authenticationMethod: 'DADOS_ABERTOS', officialUrl: 'https://jurisprudencia.tjmt.jus.br/',
        latencyMs: 0, lastCheckedAt: checkedAt, status: 'PARTIAL',
        notes: 'Portal oficial responde atualmente página de manutenção; JurisFlow não reutiliza dados antigos como atuais.',
      },
      {
      ...
      {
        courtCode: 'TJES', courtName: 'TJES - Consulta de Jurisprudência', jurisdiction: 'ES',
        jurisprudenceStatus: 'DISPONIVEL', processStatus: 'DISPONIVEL_PUBLICO',
        authenticationMethod: 'API_PUBLICA', officialUrl: 'https://sistemas.tjes.jus.br/consulta-jurisprudencia/api/search',
        latencyMs: 0, lastCheckedAt: checkedAt, status: 'READY',
        notes: 'Nova API pública oficial. PJe 2G e acervo legado são pesquisados; cada resultado é reconfirmado por ID único com inteiro teor e SHA-256.',
      },
      {
      ...
      },
      {
        ...
        jurisprudenceStatus: 'DISPONIVEL_PARCIAL', processStatus: 'DISPONIVEL_PUBLICO',
        authenticationMethod: 'PARCERIA_OFICIAL', officialUrl: 'https://tucujuris.tjap.jus.br/',
        latencyMs: 0, lastCheckedAt: checkedAt, status: 'PARTIAL',
        notes: 'Portal oficial identificado; acesso automatizado atual recebe Cloudflare HTTP 403. Fail-closed, sem bypass.',
      },
      {
        courtCode: 'TJSE', courtName: 'TJSE - Pesquisa de Jurisprudência Judicial', jurisdiction: 'SE',
        jurisprudenceStatus: 'DISPONIVEL_PARCIAL', processStatus: 'DISPONIVEL_PUBLICO',
        authenticationMethod: 'PARCERIA_OFICIAL', officialUrl: 'https://www.tjse.jus.br/Dgorg/paginas/jurisprudencia/consultarJurisprudencia.tjse',
        latencyMs: 0, lastCheckedAt: checkedAt, status: 'PARTIAL',
        notes: 'Portal judicial oficial PrimeFaces/JSF confirmado. O formulário exige Cloudflare Turnstile; JurisFlow permanece fail-closed e não gera nem contorna token.',
      },
      {
      ...
      {
        courtCode: 'TJRR', courtName: 'TJRR - Jurisprudência PrimeFaces', jurisdiction: 'RR',
        jurisprudenceStatus: 'DISPONIVEL', processStatus: 'DISPONIVEL_PUBLICO',
        authenticationMethod: 'DADOS_ABERTOS', officialUrl: 'https://jurisprudencia.tjrr.jus.br/',
        latencyMs: 0, lastCheckedAt: checkedAt, status: 'READY',
        notes: 'Busca pública JSF/PrimeFaces sem CAPTCHA, inteiro teor e PDF individual oficial por ID, com SHA-256.',
      },
      {
        courtCode: 'TJTO', courtName: 'TJTO - Jurisprudência', jurisdiction: 'TO',
        jurisprudenceStatus: 'DISPONIVEL_PARCIAL', processStatus: 'DISPONIVEL_PUBLICO',
      ...
      },
      {
        ...
        jurisprudenceStatus: 'DISPONIVEL', processStatus: 'DISPONIVEL_PUBLICO',
        authenticationMethod: 'DADOS_ABERTOS', officialUrl: 'https://jurisprudencia.tjpi.jus.br/',
        latencyMs: 0, lastCheckedAt: checkedAt, status: 'READY',
        notes: 'Busca pública JusPI com página individual por ID, ementa/acórdão completos e SHA-256 do documento oficial.',
      },
      {
      ...
      },
      {
        ...
        jurisprudenceStatus: 'DISPONIVEL', processStatus: 'DISPONIVEL_PUBLICO',
        authenticationMethod: 'API_PUBLICA', officialUrl: 'https://gateway.tjce.jus.br/sjuris/api/v1/jurisprudencia',
        latencyMs: 0, lastCheckedAt: checkedAt, status: 'READY',
        notes: 'API pública SJURIS/PJe com consulta individual e PDF autenticado confirmado por SHA-256.',
      },
      {
